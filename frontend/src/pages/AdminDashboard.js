    ...
    <>
      <div className="admin-header">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="refresh-btn" style={{ background: saved ? '#10b981' : undefined, color: saved ? 'white' : undefined, minWidth: 100 }} onClick={handleSave} disabled={saving}>
...
function CouponsTab() {
  ...
  return(
    <><div className="admin-header"><div><h1>Coupon Codes</h1><p>{coupons.length} coupons · {coupons.filter(c=>c.isActive).length} active</p></div><button className="refresh-btn" style={{fontWeight:700}} onClick={()=>setShowForm(s=>!s)}>{showForm?<><AdminIcon name="close" size={14} /> Cancel</>:<>+ New Coupon</>}</button></div>
