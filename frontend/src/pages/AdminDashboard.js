  ...
  return (
    <div className="font-field">
      ...
      <div className="font-select-wrap">
        ...
        <div className="font-preview" style={{ fontFamily: `'${value}', sans-serif` }}>
          The quick brown fox — CoreVita
        </div>
      </div>
    </div>
  );
...
function FontsTab() {
  ...
  const handleChange = (key, value) => {
    ...
    setLocal(updated);
    updateFonts(updated); // live preview
  };
  ...
  return (
    <>
      <div className="admin-header">
        <div>
          <h1>Font Settings</h1>
          <p>Changes preview instantly — click Save to apply for all visitors</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button
            ...
            style={{ background: saved ? '#10b981' : '#F5C800', color: saved ? 'white' : '#000', minWidth: 100 }}
            onClick={handleSave}
            disabled={saving}
          >
            {saving ? 'Saving...' : saved ? '✓ Saved!' : 'Save Fonts'}
          </button>
        </div>
      </div>
      ...
      <div className="fonts-panel">
        <div className="fonts-grid">
          {fontFields.map(({ key, label, description }) => (
            <FontSelect
              ...
              value={local[key] || DEFAULT_FONTS[key]}
        ...
        <div className="font-live-preview">
          ...
          <div className="preview-card">
            <div className="preview-product-title" style={{ fontFamily: 'var(--font-heading)' }}>
              CoreVita Bee Pearl Capsules
            </div>
            <div className="preview-price" style={{ fontFamily: 'var(--font-price)' }}>
              <span className="preview-current">$49.99</span>
              <span className="preview-original">$79.99</span>
            </div>
            <p className="preview-body" style={{ fontFamily: 'var(--font-main)' }}>
              CoreVita Bee Pearl is designed to <strong>restore natural vitality</strong> — naturally supporting your steady energy, recovery, and mental clarity.
            </p>
            <div className="preview-card-box" style={{ fontFamily: 'var(--font-card)' }}>
              <strong>Buy 1 + Get 1 FREE</strong> — $44.99
            </div>
            <button className="preview-btn" style={{ fontFamily: 'var(--font-button)' }}>
              ADD TO CART
            </button>
            <div className="preview-nav" style={{ fontFamily: 'var(--font-nav)' }}>
              Shop CoreVita · Contact · Track Your Order
            </div>
          </div>
        </div>
      </div>
...
function OrdersTab() {
  ...
  return (
    <>
      <div className="admin-header">
        <div><h1>Orders</h1><p>{orders.length} orders shown</p></div>
      ...
      <div className="admin-toolbar">
        ...
        <input className="admin-search" placeholder="🔍 Search order #, name, email..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      <div className="admin-content">
        <div className="message-list">
          {loading ? <div className="admin-loading">Loading orders...</div>
            : orders.length === 0 ? <div className="admin-empty"><span>📦</span><p>No orders found</p></div>
            : orders.map(order => {
              const s = ORDER_STATUS_LABELS[order.orderStatus] || { label: order.orderStatus, color: '#888' };
              return (
                <div key={order._id} className={`message-item ${selected?._id === order._id ? 'active' : ''}`} onClick={() => setSelected(order)}>
                  <div className="message-item-top">
                    <div className="message-sender">
                      ...
                      <div>
                        <p className="sender-name">{order.shippingAddress?.firstName} {order.shippingAddress?.lastName}</p>
                        <p className="sender-email">{order.guestEmail}</p>
                      </div>
                    </div>
                    <div className="message-meta">
                      <span className="message-date">{new Date(order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                    <p className="message-subject" style={{ margin: 0 }}>#{order.orderNumber}</p>
                    <span className="order-status-badge" style={{ background: s.color + '22', color: s.color, border: `1px solid ${s.color}44` }}>{s.label}</span>
                  </div>
                  <p className="message-preview">{order.items?.map(i => i.name).join(', ')} · ${order.total?.toFixed(2)}</p>
                </div>
              );
            })}
        </div>
            ...
            <div className="fade-in order-detail">
              <div className="detail-header">
                ...
                <span className="order-status-badge large"
                  style={{ background: (ORDER_STATUS_LABELS[selected.orderStatus]?.color || '#888') + '22', color: ORDER_STATUS_LABELS[selected.orderStatus]?.color || '#888', border: `1px solid ${(ORDER_STATUS_LABELS[selected.orderStatus]?.color || '#888')}44` }}>
              ...
              <div className="order-status-update">
                ...
                <div className="tracking-input-row">
                  <input className="admin-search" style={{ flex: 1 }} placeholder="Tracking number (optional, set when marking Shipped)" value={trackingInput} onChange={e => setTrackingInput(e.target.value)} />
                </div>
              </div>
            </div>
...
function CouponsTab() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const body = { code: form.code.toUpperCase().trim(), discountType: form.discountType, discountValue: parseFloat(form.discountValue), minOrderAmount: form.minOrderAmount ? parseFloat(form.minOrderAmount) : 0, maxUses: form.maxUses ? parseInt(form.maxUses) : null, expiresAt: form.expiresAt || null };
      ...
      setShowForm(false);
    } catch (err) { setFormError(err.response?.data?.message || 'Failed to create coupon'); }
  ...
  return (
    <>
      <div className="admin-header">
        <div><h1>Coupon Codes</h1><p>{coupons.length} coupons · {coupons.filter(c => c.isActive).length} active</p></div>
      ...
      {showForm && (
        <div className="coupon-form-card fade-in">
          ...
          <div className="coupon-form-grid">
            <div className="form-group"><label>Code *</label><input placeholder="e.g. SAVE20" value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} /></div>
            <div className="form-group"><label>Discount Type</label><select value={form.discountType} onChange={e => setForm({ ...form, discountType: e.target.value })}><option value="percentage">Percentage (%)</option><option value="fixed">Fixed Amount ($)</option></select></div>
            <div className="form-group"><label>Discount Value *</label><input type="number" min="0" value={form.discountValue} onChange={e => setForm({ ...form, discountValue: e.target.value })} /></div>
            <div className="form-group"><label>Min Order Amount ($)</label><input type="number" min="0" placeholder="0 = no minimum" value={form.minOrderAmount} onChange={e => setForm({ ...form, minOrderAmount: e.target.value })} /></div>
            <div className="form-group"><label>Max Uses</label><input type="number" min="1" placeholder="Leave blank for unlimited" value={form.maxUses} onChange={e => setForm({ ...form, maxUses: e.target.value })} /></div>
            <div className="form-group"><label>Expiry Date</label><input type="date" value={form.expiresAt} onChange={e => setForm({ ...form, expiresAt: e.target.value })} /></div>
          </div>
          {formError && <p className="coupon-error">{formError}</p>}
          <button className="btn-primary" onClick={handleCreate} disabled={saving}>{saving ? 'Creating...' : 'Create Coupon'}</button>
        </div>
      )}
      {loading ? <div className="admin-loading">Loading coupons...</div>
        : coupons.length === 0 ? <div className="admin-empty"><span>🏷️</span><p>No coupons yet. Create one!</p></div>
        : (
              ...
              <tbody>
                {coupons.map(c => (
                  <tr key={c._id} className={!c.isActive ? 'coupon-inactive' : ''}>
                    ...
                    <td>{c.discountType === 'percentage' ? `${c.discountValue}% off` : `$${c.discountValue.toFixed(2)} off`}</td>
                    ...
                    <td><span className={`coupon-status-badge ${c.isActive ? 'active' : 'inactive'}`}>{c.isActive ? 'Active' : 'Inactive'}</span></td>
                    <td><div className="coupon-actions"><button className="coupon-toggle-btn" onClick={() => toggleCoupon(c._id)}>{c.isActive ? 'Disable' : 'Enable'}</button><button className="coupon-delete-btn" onClick={() => deleteCoupon(c._id)}>🗑</button></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
